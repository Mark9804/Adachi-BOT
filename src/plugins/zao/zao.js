  ...
  if (db.includes(dbName, "user", "qqid", msg.uid)) {
    ...
    if (timeDiff <= 60) {
      ...
      reply = `${preGoodnightWords(
        sleepHour
      )}你已经清醒了${lastEventDurationHours}小时${lastEventDurationMinutes}分钟，记得不要勉强自己。${
        replies.good_night
      }`;
      db.update(dbName, "user", userLastData, { qqid: msg.uid, lastActivity: "sleep", time: sleepTimestamp });
    } else {
      reply = `${preGoodnightWords(
        sleepHour
      )}你已经清醒了${lastEventDurationHours}小时${lastEventDurationMinutes}分钟，${replies.good_night}`;
      db.update(dbName, "user", userLastData, { qqid: msg.uid, lastActivity: "sleep", time: sleepTimestamp });
    }
  } else {
